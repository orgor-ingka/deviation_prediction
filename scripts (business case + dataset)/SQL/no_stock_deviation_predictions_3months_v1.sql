...
# This script creates the preliminary dataset used for doing predictions of stock-related deviations at CDCs.
# This dataset consists of three months of data (October 2025 - December 2025).
# Note: This query is just a prototype and needs to be re-written before used for production.
# This dataset is described in the MIRO-board for this project: https://miro.com/app/board/uXjVJkYtT7k=/
...
FROM `ingka-ff-somdata-prod.OMDA_Analytics.unit_operation_hours_2025`), -- See the BQ-script "unit_operation_hours_2025_v1" for further details
...
CREATE OR REPLACE TABLE `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_3m_v1` AS
...
keep_first_deviation_ar AS (
  SELECT *
  FROM joining_data
  QUALIFY ROW_NUMBER() OVER (PARTITION BY country_code, order_no, prime_line_no, line_status_description, line_status_date ORDER BY event_time) = 1),

-- Create order stages
keep_first_deviation_ar2 AS (
SELECT *
      ,CASE
        WHEN line_status_description IN ('Created', 'Allocated') THEN 'ORDER PROCESSING'
        WHEN line_status_description IN ('Sent for fulfillment') THEN 'FULFILMENT'
      END AS order_stage_statuses
        FROM keep_first_deviation_ar),

-- Create revised order stages (when deviation occurrence is different from order_stage)
keep_first_deviation_ar3 AS (
SELECT *
      ,CASE
        WHEN order_stage_statuses = deviation_occurence THEN order_stage_statuses
        WHEN order_stage_statuses != deviation_occurence THEN deviation_occurence
        ELSE order_stage_statuses
      END order_stage_deviation
FROM keep_first_deviation_ar2),

final_table AS (
SELECT a.*
      ,c.* EXCEPT(item_no)
      ,d.item_id_service_level
      ,CASE
        WHEN e.item_id IS NOT NULL THEN 1 ELSE 0
      END AS ddc_item
FROM keep_first_deviation_ar3 AS a
...
LEFT JOIN (
  SELECT DISTINCT item_name
    ...
    ,hfb_no
    ,hfb_name
    ,prod_no AS prod_name_no
    ,prod_name
  FROM `ingka-ff-somdata-prod.common_dim_no_pii.vw_item_dim`) AS c
  ON a.item_id = c.item_no

  LEFT JOIN (
    SELECT ship_node, item_id, to_SL AS item_id_service_level
    FROM `ingka-ff-somdata-prod.OMDA_Analytics.item_id_service_levels`) AS d
  ON a.ship_node = d.ship_node
  AND a.item_id = d.item_id

  LEFT JOIN (
    SELECT DISTINCT item_id
    FROM `ingka-ff-somdata-prod.OMDA_Analytics.ddc_items_2025_12`) AS e
...
CREATE OR REPLACE TABLE `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_1_3m_v1` AS
SELECT * FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_3m_v1`
WHERE DATE(order_date) BETWEEN '2025-10-01' AND '2025-10-10';

CREATE OR REPLACE TABLE `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_2_3m_v1` AS
SELECT * FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_3m_v1`
WHERE DATE(order_date) BETWEEN '2025-10-11' AND '2025-10-20';

CREATE OR REPLACE TABLE `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_3_3m_v1` AS
SELECT * FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_3m_v1`
WHERE DATE(order_date) BETWEEN '2025-10-21' AND '2025-10-31';

CREATE OR REPLACE TABLE `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_4_3m_v1` AS
SELECT * FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_3m_v1`
WHERE DATE(order_date) BETWEEN '2025-11-01' AND '2025-11-10';

CREATE OR REPLACE TABLE `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_5_3m_v1` AS
SELECT * FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_3m_v1`
WHERE DATE(order_date) BETWEEN '2025-11-11' AND '2025-11-20';

CREATE OR REPLACE TABLE `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_6_3m_v1` AS
SELECT * FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_3m_v1`
WHERE DATE(order_date) BETWEEN '2025-11-21' AND '2025-11-30';

CREATE OR REPLACE TABLE `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_7_3m_v1` AS
SELECT * FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_3m_v1`
WHERE DATE(order_date) BETWEEN '2025-12-01' AND '2025-12-10';

CREATE OR REPLACE TABLE `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_8_3m_v1` AS
SELECT * FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_3m_v1`
WHERE DATE(order_date) BETWEEN '2025-12-11' AND '2025-12-20';

CREATE OR REPLACE TABLE `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_9_3m_v1` AS
SELECT * FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_3m_v1`
WHERE DATE(order_date) BETWEEN '2025-12-21' AND '2025-12-31';


CREATE OR REPLACE TABLE `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_3_1_3m_v1` AS
SELECT a.*
      ,b.* EXCEPT(country_code, business_unit, item_id)
FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_1_3m_v1` AS a
LEFT JOIN `ingka-ff-somdata-prod.OMDA_Analytics.availability_full` AS b
  ON a.ship_node = b.business_unit
  AND a.item_id = b.item_id
  AND b.modify_date_time <= a.line_status_date
-- The qualify row_number() works since we are only keeping one deviation/AR per status
QUALIFY ROW_NUMBER() OVER(
  PARTITION BY a.country_code, a.order_no, a.prime_line_no, a.line_status_description, a.ship_node, a.item_id, a.line_status_date, a.next_line_status_date
  ORDER BY b.modify_date_time DESC) = 1;

CREATE OR REPLACE TABLE `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_3_2_3m_v1` AS
SELECT a.*
      ,b.* EXCEPT(country_code, business_unit, item_id)
FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_2_3m_v1` AS a
LEFT JOIN `ingka-ff-somdata-prod.OMDA_Analytics.availability_full` AS b
  ON a.ship_node = b.business_unit
  AND a.item_id = b.item_id
  AND b.modify_date_time <= a.line_status_date
...
CREATE OR REPLACE TABLE `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_3_3_3m_v1` AS
SELECT a.*
      ,b.* EXCEPT(country_code, business_unit, item_id)
FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_3_3m_v1` AS a
LEFT JOIN `ingka-ff-somdata-prod.OMDA_Analytics.availability_full` AS b
  ON a.ship_node = b.business_unit
  AND a.item_id = b.item_id
  AND b.modify_date_time <= a.line_status_date
...
CREATE OR REPLACE TABLE `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_3_4_3m_v1` AS
SELECT a.*
      ,b.* EXCEPT(country_code, business_unit, item_id)
FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_4_3m_v1` AS a
LEFT JOIN `ingka-ff-somdata-prod.OMDA_Analytics.availability_full` AS b
  ON a.ship_node = b.business_unit
  AND a.item_id = b.item_id
  AND b.modify_date_time <= a.line_status_date
...
CREATE OR REPLACE TABLE `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_3_5_3m_v1` AS
SELECT a.*
      ,b.* EXCEPT(country_code, business_unit, item_id)
FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_5_3m_v1` AS a
LEFT JOIN `ingka-ff-somdata-prod.OMDA_Analytics.availability_full` AS b
  ON a.ship_node = b.business_unit
  AND a.item_id = b.item_id
  AND b.modify_date_time <= a.line_status_date
...
CREATE OR REPLACE TABLE `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_3_6_3m_v1` AS
SELECT a.*
      ,b.* EXCEPT(country_code, business_unit, item_id)
FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_6_3m_v1` AS a
LEFT JOIN `ingka-ff-somdata-prod.OMDA_Analytics.availability_full` AS b
  ON a.ship_node = b.business_unit
  AND a.item_id = b.item_id
  AND b.modify_date_time <= a.line_status_date
...
CREATE OR REPLACE TABLE `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_3_7_3m_v1` AS
SELECT a.*
      ,b.* EXCEPT(country_code, business_unit, item_id)
FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_7_3m_v1` AS a
LEFT JOIN `ingka-ff-somdata-prod.OMDA_Analytics.availability_full` AS b
  ON a.ship_node = b.business_unit
  AND a.item_id = b.item_id
  AND b.modify_date_time <= a.line_status_date
...
CREATE OR REPLACE TABLE `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_3_8_3m_v1` AS
SELECT a.*
      ,b.* EXCEPT(country_code, business_unit, item_id)
FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_8_3m_v1` AS a
LEFT JOIN `ingka-ff-somdata-prod.OMDA_Analytics.availability_full` AS b
  ON a.ship_node = b.business_unit
  AND a.item_id = b.item_id
  AND b.modify_date_time <= a.line_status_date
...
CREATE OR REPLACE TABLE `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_3_9_3m_v1` AS
SELECT a.*
      ,b.* EXCEPT(country_code, business_unit, item_id)
FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_2_9_3m_v1` AS a
LEFT JOIN `ingka-ff-somdata-prod.OMDA_Analytics.availability_full` AS b
  ON a.ship_node = b.business_unit
  AND a.item_id = b.item_id
  AND b.modify_date_time <= a.line_status_date
...
CREATE OR REPLACE TABLE `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_raw_3m_v1`
PARTITION BY DATE(order_date) AS
SELECT *
FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_3_1_3m_v1`
UNION ALL
SELECT *
FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_3_2_3m_v1`
UNION ALL
SELECT *
FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_3_3_3m_v1`
UNION ALL
SELECT *
FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_3_4_3m_v1`
UNION ALL
SELECT *
FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_3_5_3m_v1`
UNION ALL
SELECT *
FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_3_6_3m_v1`
UNION ALL
SELECT *
FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_3_7_3m_v1`
UNION ALL
SELECT *
FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_3_8_3m_v1`
UNION ALL
SELECT *
FROM `ingka-ff-somdata-prod.OMDA_Analytics.no_stock_deviation_predictions_3_9_3m_v1`; 